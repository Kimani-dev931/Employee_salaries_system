...
//        return "UPDATE " + tableName + " SET " + setClause.toString() + " WHERE " + idColumn + " = " + idValue;
//    }
    public static String constructUpdateStatement(String tableName, String idColumn, Object idValue, Map<String, String> fieldValues, boolean useCurrentTimestamp) {
        StringBuilder setClause = new StringBuilder();
        for (String field : fieldValues.keySet()) {
            if (setClause.length() > 0) {
                setClause.append(", ");
        ...
        // Determine the type of idValue and append it to the query accordingly
        String whereClause;
        if (idValue instanceof Integer) {
            whereClause = idColumn + " = " + idValue; // For int values
        } else if (idValue instanceof String) {
            whereClause = idColumn + " = '" + idValue + "'"; // For String values, ensure proper quoting
        } else {
            throw new IllegalArgumentException("idValue must be either Integer or String");
        }

        return "UPDATE " + tableName + " SET " + setClause.toString() + " WHERE " + whereClause;
    }
